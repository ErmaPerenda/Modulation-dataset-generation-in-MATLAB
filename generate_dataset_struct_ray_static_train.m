...
%modulationTypes=categorical(["BPSK","SSBAM","DSBAM","PAM4","QAM16"]);
        ...
        for modType=1:numModulationTypes
            ...
            for j=1:Nrandom
                ...
                for snrVal=1:size(snrsDb,2)
                    ...
                    for fd=1:size(dopplers,2)
                        disp('fd is ');
                        disp(dopplers(fd));
                        label={string(modulationTypes(modType)),snrsDb(snrVal),num2str(Ls(sps_i)),dopplers(fd),num2str(Ms(sps_i)),1,-100,-100,-100};
                        key=join(string(label),'rer');
                        key=strrep(key,'-','neg');
                        value=zeros(Ns,14*Nts,sample_length);
                        for k=1:Ns
                            ...
                            if applyRicean==true
                                y=riceanMultipath(y,fs,dopplers(fd));
                            elseif applyRayleigh==true
                                if dopplers(fd)>-1
                                y=rayleighMultipath(y,fs,dopplers(fd));
                                end
...
save('basic_dataset1024_iqenergy_static_ray_train.mat','ds','-v7.3');
