        ...
        for modType=1:numModulationTypes
            ...
            for j=1:Nrandom
                ...
                for snrVal=1:size(snrsDb,2)
                    ...
                    for sco=1:size(scos,2)
                        %disp('sco is ');
                        %disp(scos(sco));
                        label={string(modulationTypes(modType)),snrsDb(snrVal),num2str(Ls(sps_i)),0,num2str(Ms(sps_i)),1,scos(sco),-100,-100};
                        key=join(string(label),'rer');
                        key=strrep(key,'-','neg');
                        value=zeros(Ns,14*Nts,sample_length);
                        for k=1:Ns
                            ...
                            y=addClockOffset(y,fs,fc,scos(sco));
                            y=y(1:end-1);
...
save('baseline_dataset1024_iqenergy_hardware_sco_bigds.mat','ds','-v7.3');
