function [y,y_src] = helperModClassFrameGenerator(x, src_x,windowLength, stepSize, offset, sps)
...
y_src = zeros([windowLength,numFrames],class(x));
...
while startIdx + windowLength < numSamples
  ...
  xWindowed = src_x(startIdx+(0:windowLength-1),1);
  framePower = sum(abs(xWindowed).^2);
  xWindowed = xWindowed / sqrt(framePower);
  y_src(:,frameCnt) = xWindowed;
